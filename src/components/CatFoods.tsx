...
export default function CatFoods(props) {
  ...
  return (
    <div className="flex flex-wrap gap-x-[120px] gap-y-[80px]">
          ...
          <div className="w-[800px] h-[400px] bg-white rounded-[10px] flex p-[20px] justify-between">
            ...
            <div className="flex flex-col w-[48.5%] h-full">
              <div className="w-full flex justify-end">
                <div className="p-[7px] border rounded-full cursor-pointer" onClick={()=>setAddToCard(false)}>
                  <img src="./x.svg" className="w-[25px] h-[25px]"/>
                </div>
              </div>
              <div className="text-red-500 text-[28px] font-bold leading-0">{currAdding.foodName}</div>
              <div className="w-full h-full mt-[20px]">{currAdding.ingredients}</div>
              <div className="flex flex-col gap-[20px]">
                <div className="flex justify-between">
                  ...
                  <div className="flex gap-[24px]">
                    ...
                    <div className="text-[35px] font-bold">{currAddingCount}</div>
                    <div className="border rounded-full w-[50px] h-[50px] text-[30px] pt-[-50px] pl-[15px] hover:border-black cursor-pointer select-none" onClick={()=>setCurrAddingCount(currAddingCount+1)}>+</div>
                  </div>
                </div>
                <button className="bg-black rounded-full w-full text-white h-[40px]" onClick={addFoodToCard}>Add to cart</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {foods && foods.map((el) => (
          <div key={el._id} className="bg-white w-[400px] h-[350px] rounded-[10px] p-[20px]">
            <div className="rounded-[8px] border h-[220px]">
              <img src={el.image} className="w-full h-full object-cover" />
              <div className="mt-[-58px] ml-[300px] pl-[12px] leading-[42px] absolute border w-[50px] h-[50px] rounded-full bg-white text-red-400 text-[40px] cursor-pointer" onClick={()=>handlePlus(el)}>+</div>
            </div>
